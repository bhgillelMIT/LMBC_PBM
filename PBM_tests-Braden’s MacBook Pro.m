...
%% Test 4 - Uniform Binary Breakage
    ...
    %Settings
    loadresults = true;
    filepath = 'Data/Solutions/PBM_output_03-Apr-2026_23-23-30.mat'; %'Data/Solutions/PBM_output_28-Feb-2026_15-38-06.mat';
    addpath('src/');
    ...
    Nvs = 100;
    vs = logspace(log10(min(Vbs)), log10(0.9999999), Nvs);
    ...
    for it = 1:length(ts_comp)
        ...
        n_func = @(v) exp(-t) .* dirac(v-1) + (6.*t.*v) .* integral(y_func, v, 1); %exp(-t.^2) .* dirac(v-1) + 6.*t.*v .* integral(y_func, v, 1);
